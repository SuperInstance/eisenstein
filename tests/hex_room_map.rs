...
use eisenstein::{hex_directions, hex_distance, E12, HexRoomMap, MapError, Ring, RoomField};
...
mod tests {
    ...
    fn distance_matches_hex_layouts() {
        ...
        assert_eq!(E12::new(3, -2).hex_distance_to(E12::new(0, 0)), 3, "crate axial convention");
    }
    ...
    fn path_is_valid_and_adjacent() {
        ...
        for w in p.windows(2) {
            ...
            assert!(m.contains(w[0]) && m.contains(w[1]), "path walks rooms only");
        }
    ...
    fn map_temperature_reads() {
        let mut m = HexRoomMap::new();
        assert_eq!(m.map_temperature(), None, "an unread map has no temperature");
    ...
    fn deadband_rings_on_spreading_panic_and_stays_quiet_on_stable_map() {
        ...
        assert!(m.deadband_ring(0.05, 0.5).is_none(), "stable map stays quiet");
        assert!(m.deadband_ring(0.49, 0.5).is_none(), "below the band stays quiet");
